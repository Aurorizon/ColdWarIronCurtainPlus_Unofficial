...
NGame = {
	...
	--|CWIC 0.3.8| "1949.1.1.1" -> "2050.5.23.12"
	END_DATE = "1949.1.1.1",
	...
	--|CWIC 0.3.8| 10 -> 30
	LAG_DAYS_FOR_LOWER_SPEED = 30,					-- Days of client lag for decrease of gamespeed
	--|CWIC 0.3.8| 25 -> 50
	LAG_DAYS_FOR_PAUSE = 50,						-- Days of client lag for pause of gamespeed.
...
NDiplomacy = {
	...
	--|CWIC 0.3.8| 30 -> 1
	VOLUNTEERS_DIVISIONS_REQUIRED = 1,				-- This many divisons are required for the country to be able to send volunteers.
	--|CWIC 0.3.8| 2 -> 5
	...
	--|CWIC 0.3.8| 0.5 -> 1.5
	TENSION_ANNEX_CLAIM = 1.5,						-- Amount of tension generated by annexing a state you DO have claims on
	--|CWIC 0.3.8| 0 -> 1
	...
	--|CWIC 0.3.8| 0.005 -> 0.10
	TENSION_DECAY_DAILY = 0.10,					-- Each day tension decays this much for Threat sources which are no longer relevant. Replaces TENSION_DECAY as of 1.12.0
	--|CWIC 0.3.8| 1.0 -> 0.25
	TENSION_SIZE_FACTOR = 0.25,						-- All action tension values are multiplied by this value
	--|CWIC 0.3.8| "1936.1.1.12" -> "1999.1.1.12"
	TENSION_TIME_SCALE_START_DATE = "1999.1.1.12",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	--|CWIC 0.3.8| -0.005 -> 0.00
	TENSION_TIME_SCALE_MONTHLY_FACTOR = 0.00,		-- Timed tension scale will be modified by this amount starting with TENSION_TIME_SCALE_START_DATE
	--|CWIC 0.3.8| 0.25 -> 0.0
	TENSION_TIME_SCALE_MIN = 0.0,					-- Timed tension scale won't decrease under this value
	--|CWIC 0.3.8| -5 -> 0
	TENSION_GUARANTEE = 0,
	TENSION_FACTION_JOIN = 4,
	TENSION_JOIN_ATTACKER = 2,						-- scale of the amount of tension added when country joins attacker side
	--|CWIC 0.3.8| 0.25 -> 0.0
	TENSION_PEACE_FACTOR = 0.0,					-- scale of the amount of tension (from war declaration) reduced when peace is completed.
	--|CWIC 0.3.8| -1 -> 0
	TENSION_LIBERATE = 0,							-- Amount of tension generated by liberating a country.
	...
	--|CWIC 0.3.8| 0.40 -> 1.0
	TENSION_CAPITULATE = 1.0,						-- Scale of the amount of tension created by a countries capitulation.
	...
	--|CWIC 0.3.8| 200 -> 5000
	ASSUME_FACTION_LEADERSHIP_PP_COST = 5000,				-- Political power cost to assume faction leadership
...
NCountry = {
	...
	--|CWIC 0.3.8| 0.015 -> 0.008
	POPULATION_YEARLY_GROWTH_BASE = 0.008,			-- basic population growth per year, used for monthly manpower gain
	...
	--|CWIC 0.3.8| 0.15 -> 2.0
	ARMY_SCORE_MULTIPLIER = 2.0,					-- Based on number of armies.
	--|CWIC 0.3.8| 1.0 -> 25.0
	NAVY_SCORE_MULTIPLIER = 25.0,					-- Based on number of navies.
	--|CWIC 0.3.8| 0.1 -> 0.01
	...
	--|CWIC -.3.8| 0.8 -> -2
	MIN_COUP_SUCCESS_STABILITY = -2,				-- Max stability when coup will happen
	...
	--|CWIC 0.3.8| 0.1 -> 0.5
	AIR_VOLUNTEER_BASES_CAPACITY_LIMIT = 0.5,		-- Ratio for volunteer planes available for sending in relation to receiver air base capacity
	...
	--|CWIC 0.3.8| 2 -> 4
	BASE_FUEL_GAIN_PER_OIL = 4,						-- base amount of fuel gained hourly per excess oil
...
NProduction = {
	...
	--|CWIC 0.3.8| 1 -> 0.7
	INFRA_MAX_CONSTRUCTION_COST_EFFECT = 0.7, 		-- Building in a state with higher infrastructure will reduce the cost of shared buildings.
...
NTechnology = {
	...
	--|CWIC 0.3.8| 0.5 -> 0.15
	MAX_TECH_SHARING_BONUS = 0.15, 			-- Max technology sharing bonus that can be applied instantly
...
NBuildings = {
	--|CWIC 0.3.8| 5.0 -> 100.0
	ANTI_AIR_SUPERIORITY_MULT = 100.0,	-- How much air superiority reduction to the enemy does our AA guns? Normally each building level = -1 reduction. With this multiplier.
	--|CWIC 0.3.8| 15 -> 200
	MAX_BUILDING_LEVELS = 200,			-- Max levels a building can have.
	--|CWIC 0.3.8| 200 -> 50
	...
	--|CWIC 0.3.8| 100 -> 0
	ROCKETSITE_CAPACITY_MULT = 0,		-- Each level of rocketsite building multiplied by this, gives capacity (max operational value). Value is int. 1 for each rocket.
	...
	--|CWIC 0.3.8| 200 -> 400
	RADAR_RANGE_MAX = 400,				-- Range is interpolated between building levels 1-15.
	...
	--|CWIC 0.3.8| 3 -> 5
	SUPPLY_PORT_LEVEL_THROUGHPUT = 5,   -- supply throughput per level of naval base
	--|CWIC 0.3.8| 25 -> 42, CWICP 25 -> 306
	MAX_SHARED_SLOTS = 306,				-- Max slots shared by factories
	...
	--|CWIC 0.3.8| 0.2 -> 0.0
	INFRASTRUCTURE_RESOURCE_BONUS = 0.0, -- multiplicative resource bonus for each level of (non damaged) infrastructure
	--|CWIC 0.3.8| 0.2 -> 0.0
	SUPPLY_ROUTE_RESOURCE_BONUS = 0.0,   -- multiplicative resource bonus for having a railway/naval connection to the capital
...
NMilitary = {
	...
	--|CWIC 0.3.8| 0.060 -> 0.025
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.025,       -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	--|CWIC 0.3.8| 0.053 -> 0.025
	LAND_COMBAT_ORG_DAMAGE_MODIFIER = 0.025,       -- global damage modifier
	--|CWIC 0.3.8| 0.032 -> 0.08
	LAND_AIR_COMBAT_STR_DAMAGE_MODIFIER = 0.08,    -- air global damage modifier
	--|CWIC 0.3.8| 0.032 -> 0.12
	LAND_AIR_COMBAT_ORG_DAMAGE_MODIFIER = 0.12,    -- global damage modifier
	LAND_AIR_COMBAT_MAX_PLANES_PER_ENEMY_WIDTH = 3, -- how many CAS/TAC can enter a combat depending on enemy width there
	--|CWIC 0.3.8| 2 -> 6
	LAND_COMBAT_STR_ARMOR_ON_SOFT_DICE_SIZE = 6,   -- extra damage dice if our armor outclasses enemy
	...
	--|CWIC 0.3.8| 0.0 -> 0.05
	BASE_CAPTURE_EQUIPMENT_RATIO = 0.05,			-- after a successful land combat, ratio of the equipments that are being captured/salvaged from enemy's lost equipment
...
NAir = {
	...
	--|CWIC 0.3.8| 800 -> 10000
	AIR_WING_MAX_STATS_SPEED = 10000,
	--|CWIC 0.3.8| 100 -> 1000
	AIR_WING_MAX_STATS_BOMBING = 1000,
	--|CWIC 0.3.8| NO MODIFIED
	AIR_WING_MAX_SIZE = 1000, 							-- Max amount of airplanes in wing
	--|CWIC 0.3.8| 100 -> 50, CWICP 100 -> 25
	AIR_WING_AVERAGE_SIZE = 25, 						-- Eyeballed average amount of airplanes in wing. Used when calculating air volunteer.
	--|CWIC 0.3.8| NO MODIFIED
	...
	--|CWIC 0.3.8| 0.45 -> 0.6
	COMBAT_BETTER_AGILITY_DAMAGE_REDUCTION = 0.6, 		-- How much the better agility (than opponent's) can reduce their damage to us.
	...
	--|CWIC 0.3.8| 1.0 -> 2
	NAVAL_STRIKE_DAMAGE_TO_STR = 2,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
	...
	--|CWIC 0.3.8| 0.75 -> 10.0
	STRATEGIC_BOMBER_NUKE_AIR_SUPERIORITY = 10.0,		-- How much air superiority is needed for a tactical bomber to be able to nuke a province
	...
	--|CWIC 0.3.8| 0.05 -> 0.02
	NAVAL_COMBAT_EXTERNAL_PLANES_JOIN_RATIO = 0.02,		-- Max planes that can join a combat comparing to the total strength of the ships
	...
	--|CWIC 0.3.8| 0.1 -> 0.5 and 0.1 -> 0.8
	CAS_NIGHT_ATTACK_FACTOR = 0.5,                      -- CAS damaged get multiplied by this in land combats at night
...
NNavy = {
	...
	--|CWIC 0.3.8| 168 -> 72
	NAVAL_INVASION_PREPARE_HOURS = 72,								-- base hours needed to prepare an invasion
	...
	--|CWIC 0.3.8| 0.25 -> 0.15
	SHORE_BOMBARDMENT_CAP = 0.15,
	...
	--|CWIC 0.3.8| 0.05 -> 0.15
	LIGHT_GUN_ATTACK_TO_SHORE_BOMBARDMENT							= 0.15, -- light gun attack value is divided by this value * 100 and added to shore bombardment modifier
	...
	--|CWIC 0.3.8| 11 -> 0.002
	SUBMARINE_REVEAL_BASE_CHANCE 									= 0.002,		-- Base factor for submarine detection. It's modified by the difference of a spotter's submarines detection vs submarine visibility. Use this variable for game balancing. setting this too low will cause bad spotting issues.
...
NTrade = {
	...
	--|CWIC 0.3.8| 1 -> 5
	RELATION_TRADE_FACTOR = 5,					-- Trade factor is modified by Opinion value times this
...
NAI = {
	...
	--|CWIC 0.3.8| 0.75 -> 0.0
	DIPLOMACY_CREATE_FACTION_FACTOR = 0.0,		-- Factor for AI desire to create a new faction. Val < 1.0 makes it less likely to create than to join.
	--|CWIC 0.3.8| 60 -> 0
	...
	--|CWIC 0.3.8| 100.0 -> 0.0
	DIPLOMACY_BOOST_PARTY_COST_FACTOR = 0.0,	-- Desire to boost party popularity subtracts the daily cost multiplied by this
	--|CWIC 0.3.8| 5.0 -> 0.0
	DIPLOMACY_IMPROVE_RELATION_COST_FACTOR = 0.0,-- Desire to boost relations subtracts the cost multiplied by this
	--|CWIC 0.3.8| 0.1 -> 0.0
	...
	--|CWIC 0.3.8| 5.0 -> 1.5
	RESEARCH_BONUS_FACTOR = 1.5,                -- To which extent AI should care about research speed bonuses
	...
	--|CWIC 0.3.8| 2 -> 0
	MAX_AHEAD_RESEARCH_PENALTY = 0,             -- max ahead of tiem penalty ai will pick ever
	--|CWIC 0.3.8| 4.0 -> 10.0, 4.0 -> 9999999
	...
	--|CWIC 0.3.8| 10 -> 0
	ROCKET_MIN_ASSIGN_SCORE = 0,				-- Minimum total score for region to be considered for rocket air missions
	--|CWIC 0.3.8| 50 -> 0
	ROCKET_MIN_PRIO_ASSIGN_SCORE = 0,			-- Minimum total score for region to be considered for critical rocket air missions
	--|CWIC 0.3.8| 0.5 -> 0
	ROCKET_ASSIGN_SCORE_REDUCTION_PER_ASSIGNMENT = 0, -- each assigned rocket reduces the score of a region by this amount
	...
	--|CWIC 0.3.8| 0.0025 -> 0.2
	NUM_SILOS_PER_CIVILIAN_FACTORIES = 0.2,					-- ai will try to build a silo per this ratio of civ factories
...
NFocus = {
	...
	--|CWIC 0.3.8| 10 -> 15
	MAX_SAVED_FOCUS_PROGRESS = 15,				-- This much progress can be saved while not having a focus selected
},

NOperatives = {
	--|CWIC 0.3.8| 30 -> 200
	AGENCY_CREATION_DAYS = 200,						-- Number of days needed to create an intelligence agency
	--|CWIC 0.3.8| 30 -> 100
	AGENCY_UPGRADE_DAYS = 100,						-- Number of days needed to upgrade an intelligence agency
	--|CWIC 0.3.8| 5 -> 15
	AGENCY_CREATION_FACTORIES = 15,					-- Number of factories used to create an intelligence agency
	...
	--|CWIC 0.3.8| 5 -> 24
	AGENCY_UPGRADE_PER_OPERATIVE_SLOT = 24,			-- Number of upgrade needed to unlock an additional operative slot
	MAX_OPERATIVE_SLOT_FROM_AGENCY_UPGRADES = 1,	-- max operative slots gained from upgrades
	--|CWIC 0.3.8| 30 -> 90
	AGENCY_OPERATIVE_RECRUITMENT_TIME = 90,			-- Number of days to wait to have operative to recruit when an operative slot first becomes available
...
NSupply = {
	-- 不要太相信注释, 自己验证一下最好
	-- 对于"最后一公里"运输阶段, 从补给中心启程输向范围内province的补给量是相同的, 最终province能收到多少补给会有所衰减
	
	-- Modified
	-- lol,High-speed rail~
	MAX_RAILWAY_LEVEL = 16, -- 5->16 随着等级提高, 铁路纹理会随之改变, 每一等级对应一种纹理
	...
	-- 定义用以计算首都补给上限，其也会用于计算其他节点的补给上限，具体取决于它们与首都补给中心的连接程度。使用公式：
	...
	-- 首都补给 = 首都基础补给 + (民用工厂数量x首都补给民用工厂系数) + (军用工厂数量x首都补给军用工厂系数) + (海军船坞数量x首都补给海军船坞系数)
	CAPITAL_SUPPLY_BASE = 5.0, -- 首都的基础补给 5.0
	CAPITAL_SUPPLY_CIVILIAN_FACTORIES = 0.15, 	-- 0.3->0.15 首都补给的总民用工厂系数, 即每一个民用工厂提供的补给量
	CAPITAL_SUPPLY_MILITARY_FACTORIES = 0.3, 	-- 0.6->0.3 首都补给的总军用工厂系数, 即每一个军用工厂提供的补给量
	CAPITAL_SUPPLY_DOCKYARDS = 0.2, 			-- 0.4->0.2 首都补给的总海军船坞系数, 即每一个海军船坞提供的补给量

	-- Modified
	-- defines that are used for supply reach for capital. 这个定义用于首都的补给范围.
	-- 供应流量将从 INITIAL_SUPPLY_FLOW 开始, 随着自补给中心出发通过的省份衰减(取决于距补给中心的距离, 地形等因素)
	-- 补给达到 >= 1.0 被认为是"完美的", 将能够完全支持该province的部队(假如您的补给不过剩)
	CAPITAL_INITIAL_SUPPLY_FLOW = 5.0,              -- 5.0->25.0 首都的初段补给流量
	CAPITAL_STARTING_PENALTY_PER_PROVINCE = 0.4,    -- 0.5->0.4 当补给离开其来源时, 将增加起始惩罚(受地形等因素影响)
	CAPITAL_ADDED_PENALTY_PER_PROVINCE = 1.2,       -- 当补给运输到远离其补给来源时的追加惩罚
	-- 起始惩罚: 初段补给和后续补给进入下一Province时需要减少的基础补给值
	-- 追加惩罚: 初段补给进入第一个Province后进入下一Province时, 每经过一个Province需要追加的补给减少值
	-- 起始惩罚和追加惩罚均受到地形因素的影响

	-- Modified
	-- defines that are used for supply reach for built nodes
	NODE_INITIAL_SUPPLY_FLOW = 2.8,				-- 建造的补给中心的初段补给流量 2.8
	NODE_STARTING_PENALTY_PER_PROVINCE = 0.30, 	-- 0.50->0.30
	NODE_ADDED_PENALTY_PER_PROVINCE = 0.40,    	-- 0.70->0.40

	-- Modified
	-- defines that are used for supply reach for dockyards
	NAVAL_BASE_INITIAL_SUPPLY_FLOW = 3.3,		-- 建造的海军基地的初段补给流量 3.3
	NAVAL_BASE_STARTING_PENALTY_PER_PROVINCE = 0.62, -- 0.84->0.62
	NAVAL_BASE_ADDED_PENALTY_PER_PROVINCE = 0.70,    -- 1.10->0.70

	-- Node Flow (i.e. province caps) increase by this amount per railway level of the node's bottleneck
	NODE_FLOW_BONUS_PER_RAIL_LEVEL = 0.70,     -- 0.34->0.70 每级铁路给予节点的补给额外增益
	
	-- INITIAL_SUPPLY 初段补给, 即由首都补给中心->本地补给中心(包括普通补给中心和海军基地补给中心)的补给流量
	-- 初段补给 = 铁路瓶颈等级x@NODE_FLOW_BONUS_PER_RAIL_LEVEL + 补给中心类型的基础值 + 摩托化水平修正
	-- 有关初段补给:
	-- 1.提升基建等级不能提升由首都补给中心运送到本地补给中心的补给
	-- 2.初段补给不受"补给中心分发补给上限限定"的影响
	-- 3.提升摩托化水平可以直接提升"初段补给"的值
	

	-- 当河流用于节点间的"管道"时, 其等效于 @Level=RIVER_RAILWAY_LEVEL 级铁路. 当存在铁路时, 优先使用铁路作为"管道".
	-- 这一等效有两个含义: 1.其对补给中心分发补给上限的影响仅为 @Level=RIVER_RAILWAY_LEVEL 级铁路; 2. 其对后续计算"初段补给"的影响仅为 @Level=RIVER_RAILWAY_LEVEL 级铁路.
	...
	-- Modified
	FLOATING_HARBOR_BASE_SUPPLY = 25.0, -- 15.0->25.0 supply given by a floating harbor
	...
	SUPPLY_FLOW_DROP_REDUCTION_AT_MAX_INFRA = 0.50, -- 0.30->0.50 最大基础设施水平将以此比率减少补给流量下降
	SUPPLY_FLOW_PENALTY_CROSSING_RIVERS = 0.15, -- 0.20->0.15 补给中心分发补给时过河的额外惩罚
	...
	SUPPLY_FLOW_DIST_LOGISTICS_FALLOFF_MIDPOINT = 2.3, -- Sigmoid曲线拐点
	SUPPLY_FLOW_DIST_LOGISTICS_FALLOFF_SCALAR = 0.9, -- 距离影响下的最大调整处罚
	SUPPLY_FLOW_DIST_LOGISTICS_FALLOFF_MIN_PENALTY_SCALE = 0.25, -- 曲线的减少惩罚不低于这个极限

    -- Modified
	-- 一个补给中心为完全摩托化时提升的额外补给值. 该值将与 XXX_INITIAL_SUPPLY_FLOW 相加.
	SUPPLY_HUB_FULL_MOTORIZATION_BONUS = 3.2,            -- 2.2->3.2
	-- 一个补给中心若要完全摩托化需要的卡车数
	SUPPLY_HUB_FULL_MOTORIZATION_TRUCK_COST = 160.0,     -- 80.0->160.0
	...
	-- 对于一个补给站中的每一个额外的机动化水平对下一等级的运输, 减少最大额外增益@SUPPLY_HUB_MOTORIZATION_MARGINAL_EFFECT_DECAY
	SUPPLY_HUB_MOTORIZATION_MARGINAL_EFFECT_DECAY = 1.6,
	-- 在原版中, 依照初段补给计算我们不难得出, 一个满级铁路的普通补给中心仅能在当前State提供: 5x0.34 + 2.8 + 2.2 = 6.7的补给，但该补给可与State人口/胜利点所提供的补给相加

    
	-- Modified
	-- 用于计算铁路的"流量".
	RAILWAY_BASE_FLOW = 12.5, 		         -- 10.0->12.5 当一个节点通过铁路连接到其首都/海军节点时，铁路提供的基本"流量"值 10.0
	RAILWAY_FLOW_PER_LEVEL = 7.5, 	         -- 5.0->7.5 每一级铁路提供的额外"流量"值
	-- 铁路"流量"的计算公式为: 铁路"流量" = 铁路基本"流量"值 + 铁路额外等级x每级铁路的额外"流量"值
	RAILWAY_FLOW_PENALTY_PER_DAMAGED = 45.0,  -- 5.0->45.0 损坏铁路的"流量"减小惩罚, 铁路损坏惩罚 = 铁路损坏惩罚值 x (损坏province途径数/总province途径数)
	RAILWAY_MIN_FLOW = 7.5, 		         -- 5.0->7.5 铁路"流量"可被减小到的最小值
    
	-- Modified
	-- used for calculating "flow" from a naval node to another naval node when it is connected via a convoy route
	-- 当通过海路运输连接时, 用于计算从一个海军节点到另一个海军节点的"流量"
	-- NAVAL_BASE_MAX_SUPPLY_FLOW_FACTOR = 0.9,
	-- 父节点的流被分解为该比率, 即海路运输时最大能从父节点运输的流量 = 父节点流量x@NAVAL_BASE_MAX_SUPPLY_FLOW_FACTOR
	-- 该比率仅限于海运到非核心地区, 在原版中, 3级港口
	NAVAL_BASE_FLOW = 15.0,                   -- 最大"流量"受到该基础值加额外等级提供的"流量"的限制
	NAVAL_FLOW_PER_LEVEL = 5.5,               -- 5.0->5.5 max output/input of a naval node is limited by previous base value + this define per its level
	-- 注意:
	-- 当能保证制海权时, 海路运输的瓶颈通常大于陆路运输. 因此, 沿海补给中心往往优先选择使用海路运输, 若要避免使用海路运输可通过"海上运输线禁航"使其转为使用陆路运输

	SUPPLY_NODE_MIN_SUPPLY_THRESHOLD = 1.0,  -- 当补给中心所受补给流量小于1时, 其将被视作与补给网络断开连接,即置于0 -- Currently unused?

	INFRA_TO_SUPPLY = 0.6,							-- 0.3->0.6 每一级基础设施提供的本地补给量
	-- 注意, 胜利点提供的本地补给只要占领了胜利点所在的"province"既可立即获得并添加到"总本地补给"中.
	VP_TO_SUPPLY_BASE = 0.0,					    -- 0.2->0.0 胜利点提供的基础本地补给量, 与胜利点数值大小无关
	VP_TO_SUPPLY_BONUS_CONVERSION = 0.25,			-- 0.05->0.25 胜利点提供的额外本地补给量, 其值为 胜利点大小x@VP_TO_SUPPLY_BONUS_CONVERSION , 取最接近的整数
	SUPPLY_FROM_DAMAGED_INFRA = 0.3,               -- 0.15->0.3 每一级受损的基础设施提供的当地补给量
	SUPPLY_BASE_MULT = 0.2,							-- 补给的基本乘数
	SUPPLY_DISRUPTION_DAILY_RECOVERY = 1.5,		    -- 当"管道"修复完毕后，节点会每天自积累的中断中恢复该数值

	-- 铁路在被敌人占领时将处于冷却状态, 并且在冷却期间不可用.
	RAILWAY_CONVERSION_COOLDOWN = 10, 			-- 非核心地区铁路冷却时间
	RAILWAY_CONVERSION_COOLDOWN_CORE = 5,		-- 核心地区铁路冷却时间
	RAILWAY_CONVERSION_COOLDOWN_CIVILWAR = 0,	-- 内战中的铁路冷却时间

	DEFAULT_STARTING_TRUCK_RATIO = 1.5, 	-- 与Countries的需求量相比, 其将得到此比率的起始卡车
	DEFAULT_STARTING_TRAIN_RATIO = 1.2, 	-- 1->1.2 与Countries的需求量相比, 其将得到此比率的起始火车

	SUPPLY_POINTS_PER_TRAIN = 2.5,  -- old default 1.25 -- 1.0->2.5 一列火车能容纳的供给量(列车将供应从首都分配到供应节点)
	NUM_RAILWAYS_TRAIN_FACTOR = 0.03, -- 列车使用量将按补给节点和首都补给中心之间的铁路距离乘以该系数来确定

	BASE_SUPPLY_MULT_FOR_TRUCK_DEFAULT_BUFFER = 1.0,  -- 潜在卡车使用量的所需缓冲区的初始值
	BASE_SUPPLY_MULT_FOR_TRUCK_MIN_BUFFER = 0.0, -- 缓冲比的最小值与最大值
	BASE_SUPPLY_MULT_FOR_TRUCK_MAX_BUFFER = 100.0,

	TRUCK_ATTRITION = 0.003, -- 卡车的基础损耗
	TRUCK_ATTRITION_FACTOR = 0.65, -- 卡车总损耗的比例

	BASE_TRUCK_HP = 120.0, -- 100.0->120.00 卡车基础HP
	TRUCK_HP_PER_ARMOR = 2, -- 每点装甲值提供的卡车HP

	BASE_TRAIN_HP = 120.0, -- 100.0->120.00 火车基础HP
	TRAIN_ARMOR_TARGETING_WEIGHT = 0.01, -- 对于每一装甲值获得的生命值, 更有可能成为敌方轰炸机的目标
	...
	MIN_TRAIN_SUPPLY_FACTOR = 0.0, -- 0.5->0.0 库存中有0列火车时应用此惩罚系数, 在满足火车数量需求时放大至1.0
	MIN_TRAIN_REQUIREMENT = 2, -- 如果火车总需求小于等于此值, 即便库存不足, 也不会施加任何供应惩罚

	SUPPLY_FLOW_REDUCTION_THRESHOLD = 0.1, -- 如果供应流量低于此值, 则不被允许
	...
	-- 注意, 这些基础值并不能随着建造补给中心或铁路而增加.
	-- 基础值不定地存在于不同的"State"中，可以视作固定常量.
	CAPITAL_NODE_BASE_SUPPLY_ADD = 0, -- 首都节点的基础补给修正
	BUILT_NODE_BASE_SUPPLY_ADD = 0.6, -- 建造的节点的基础补给修正
	LOCAL_NODE_BASE_SUPPLY_ADD = 0.5, -- 本地节点的基础补给修正
	NAVAL_NODE_BASE_SUPPLY_ADD = 0.3, -- 海军节点的基础补给修正
	-- ~end

	-- 部队如果收到足够的补给流量, 它缓慢增加并储备超过>100%的供应, 这取决于他们的补给上限
	-- 否则, 他们将每天失去高达100%的供应, 这取决于补给流量有多糟糕
	ARMY_SUPPLY_RATIO_STARTING_GAIN = 0.0,			-- 部队补给起始获取比率为
	ARMY_SUPPLY_RATIO_SPEED_GAIN_PER_HOUR = 0.01,	-- 每小时部队获取的补给速率的比率
	ARMY_MAX_SUPPLY_RATIO_GAIN_PER_HOUR = 0.15,		-- 每小时部队最大获取的补给速率

	MIN_SURRENDER_LIMIT_TO_MOVE_SUPPLY_CAPITAL = 0.15, -- Country需要高于15%的投降倾向才能转移其首都补给中心
	COOLDOWN_DAYS_AFTER_MOVING_SUPPLY_CAPITAL = 30, -- 自上一次转移首都补给中心后的冷却时间
	DAYS_TO_START_GIVING_SUPPLY_AFTER_MOVING_SUPPLY_CAPITAL = 7,  -- 该Country将在转移其首都补给中心该天数后开始获得补给
	DAYS_TO_START_GIVING_FULL_SUPPLY_AFTER_MOVING_SUPPLY_CAPITAL =  21, -- 该Country将在转移其首都补给中心该天数后达到最大补给

	MIN_DIFF_FOR_AUTO_UPDATING_EXISTING_RAILWAYS = 5, -- 在修建铁路时, 如果新铁路与现有铁路足够接近, 系统将倾向于更新现有铁路

	-- NOT CURRENT VERSION LOCAL_SUPPLY_PER_AIR_MISSION = 0.5, -- 0.2->0.5 每架执行"空投补给"任务的运输机以全效率执行任务时提供的本地补给量

	-- 增援依据与首都补给中心的距离, 同时以下定义用于计算增益时间
	SUPPLY_PATH_MAX_DISTANCE = 15,	-- 花费的最大时间
	RAILWAY_DISTANCE_FACTOR_FOR_REINFORCEMENT_SPEED = 0.3, -- 总铁路运输距离的时间因数
	TRUCK_DISTANCE_FACTOR_FOR_REINFORCEMENT_SPEED = 0.01, -- 总卡车运输距离的时间因数
	NAVAL_DISTANCE_FACTOR_FOR_REINFORCEMENT_SPEED = 0.08, -- 总海路运输距离的时间因数

	ALERT_VERY_LOW_SUPPLY_LEVEL = 0.4,		-- 0.2->0.4	   -- 在补给数值多少时, 会显示补给水平低或补给水平非常低的警告, 该值以实际补给/需求补给的百分比来表示
	ALERT_LOW_SUPPLY_LEVEL = 0.6,			-- 0.5->0.6

	AI_FRONT_MINIMUM_UNITS_PER_PROVINCE_FOR_SUPPLY_CALCULATIONS = 1,    -- 当AI评估战场前线的补给受限时, 会保证前线每个省份至少有一个师
	AI_FRONT_DIVISIONS_PER_SUPPLY_POINT = 1.0, 					-- 对于每个可用的补给点, AI应该考虑它可以补给多少个师
	AI_FRONT_MAX_UNITS_ENEMY_COUNT_FACTOR = 1.2, 				-- 确保AI前线的最大单位数量至少是敌人数量的1.2
	SUPPLY_THRESHOLD_FOR_ARMY_ATTRITION = 0.35, 				-- 军队低于该值的补给比率时将会受到损耗惩罚
	NUMBER_OF_SHOWN_SUPPLY_SOURCES_IN_SUPPLY_MAPMODE = 3, 		-- 在故障提示中显示的补给流量资源数量
	ESTIMATED_DIVISION_WEIGHT_FOR_SUPPLY_ESTIMATIONS_GUI = 1.0,	-- 师补给花费用于评估前线提示的权重
	AVAILABLE_MANPOWER_STATE_SUPPLY = 0.5,						-- 0.18->0.5 地区内每1M人力提供的补给量
	NON_CORE_MANPOWER_STATE_SUPPLY = 0.2,						--Factor for population sttate supply when controlled by an occupier (NO TAKE FOOD)
	STORED_SUPPLY_CONSUMPTION_RATE_FACTOR = 0.75,				-- 补给储量的花费乘数倍率(更多/更少地缓和)
},
...
-- OUT OF DATA DEFINES IN CWIC VANILLA:
--
-- NDefines.NAI.AIR_WING_REINFORCEMENT_LIMIT = 60
--
-- NDefines.NAir.COMBAT_STAT_IMPORTANCE_SPEED = 2
-- NDefines.NAir.COMBAT_STAT_IMPORTANCE_AGILITY = 1
-- NDefines.NAir.COMBAT_DAMAGE_STATS_MULTILIER = 0.3
-- NDefines.NAir.COMBAT_STACK_LIMIT = 3
--
...
--
-- NDefines.NNavy.SEA_AIR_COMBAT_MAX_WINGS_ON_STACK = 75
--
